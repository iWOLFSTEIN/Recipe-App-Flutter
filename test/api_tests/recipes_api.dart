...
class RecipesApis {
  ...
  static void _testMockRecipesApi() =>
      group('Recipes apis test with mockito', () {
        ...
        test('Testing get recipes by tag api', () async {
          // Mock server
          when(mockDio.get(any)).thenAnswer((_) async => Response(
              data: recipesByTag,
              statusCode: 200,
              requestOptions: RequestOptions(path: '/recipes/tag')));

          // Mock client
          final response = await mockDio.get('/recipes/tag');

          // Testing
          expect(
              RecipesResponse.fromJson(response.data as Map<String, dynamic>),
              RecipesResponse.fromJson(recipes));
          verify(mockDio.get('/recipes/tag')).called(1);
        });
      });
